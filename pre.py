...
def changeView(originalImg):
    ...
    contoursGray, hierarchyGray = cv.findContours(edgesGray, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    contoursS, hierarchyS = cv.findContours(edgesS, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    if report:
    ...
    #Transform
    resultImage = cv.warpPerspective(originalImg, transMat, (int(newWidth), int(newHeight)))
    if debug == True:
...
if __name__ == "__main__":
	...
	img = cv.imread(args.input_file);
	changeview = changeView(img)

	cv.imshow('asdf', changeView)
	cv.waitKey(0)